...
class QuizStorageService {
  ...
  Future<void> saveSession(QuizSession session, {String? pdfHash}) async {
    ...
    await _updateWeeklyStats(
      session,
      pdfHash: pdfHash,
    ); // step 4: update leaderboard stats
  ...
  double computeLeaderboardScore() {
    ...
    final lifetimeTotalAnswered =
        _stats.get(keyTotalAnswered, defaultValue: 0) as int;
      ...
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'accuracyPercent': weekly['weeklyAnswered'] > 0
                ? (weekly['weeklyCorrect'] / weekly['weeklyAnswered']) * 100
                : 0.0,
  ...
  Future<void> _updateLifetimeStats(QuizSession session) async {
    ...
    final prevBest =
        _stats.get(keyHighestAccuracy, defaultValue: 0.0) as double;
